...
- [목차](#%eb%aa%a9%ec%b0%a8)
  ...
  - [프로세스](#%ed%94%84%eb%a1%9c%ec%84%b8%ec%8a%a4)
    - [프로세스는?](#%ed%94%84%eb%a1%9c%ec%84%b8%ec%8a%a4%eb%8a%94)
  ...
  - [CPU 스케줄링](#cpu-%ec%8a%a4%ec%bc%80%ec%a4%84%eb%a7%81)
    - [동시-Simultaneous](#%eb%8f%99%ec%8b%9c-simultaneous)
    - [선점과 비선점](#%ec%84%a0%ec%a0%90%ea%b3%bc-%eb%b9%84%ec%84%a0%ec%a0%90)
    - [IO](#io)
    - [CPU Bound vs IO Bound](#cpu-bound-vs-io-bound)
...
2017년 6월 이슈가 되었던 인텔 CPU의 [멜트다운](https://ko.wikipedia.org/wiki/%EB%A9%9C%ED%8A%B8%EB%8B%A4%EC%9A%B4_(%EB%B2%84%EA%B7%B8))도 이 맥락에서 이해가 가능하다. 원래 유저는 커널모드의 Instruction을 실행할 수 없다. 그러나 CPU 실행 중에 간접적으로 커널 영역의 값을 알아낼 수 있는 방법이 알려졌고, 멜트다운으로 명명되었다. 자세한 내용은 구글에 검색하면 잘 나온다. 보다보면 CPU 동작 방식을 유추해 볼 수 있다. 캐시, 레지스터, 커널 등이 언급된다.
...
![시스템콜](https://user-images.githubusercontent.com/40727649/68071677-015e0d80-fdc0-11e9-8483-f5721978acb3.PNG)
...
네트워크 역시 Communication이다. 다른 호스트의 프로세스와 데이터를 주고 받는다. 운영체제는 Communication을 위한 여러 인터페이스를 제공한다.
...
- Data 영역엔 전역변수나 Object가 존재한다. 예) 메서드, 인스턴스, static 변수, 글로벌 변수 ...
...
프로세스는 "처음" 메모리에 등록될 때 자신의 메모리 영역이 결정된다. 초기 할당받은 영역을 넘어서는 메모리에 접근하려 하면 다른 프로세스의 메모리 영역을 침범하게 된다. 커널은 이것을 감지하고 침범하지 못하게 제어한다. 
메모리 초과 형태 중,  
Data(Heap)영역과 관련된 것은 `OutOfMemoryException`다. 인스턴스가 과도하게 많을 경우 발생한다.  
Stack영역과 관련된 것은 `StackOverFlow`가 있다. 실행해야 할 명령들이 과도하게 쌓였을 경우 발생한다. 재귀호출, 순환호출시 발생할 확률이 높다.
...
다른 예를 들어보자. 당신은 어느 회사의 막내사원이다. 팀장님은 아주 무서운 분이시다. 업무시간에 딴짓하는 게 걸리면 무지하게 혼난다. 그러나 당신은 카카오톡도 해야하고 쇼핑도 해야한다. 팀장님이 안 볼때 신나게 카카오톡을 한다. 팀장님이 일어나면 냉큼 알트탭으로 일하는 척을 한다. 팀장님이 보기에 당신은 열심히 일하고 있다. 팀장님이 자리에 앉으면 다시 쇼핑을 시작한다. 팀장님이 일어나면 알탭. 앉으면 딴짓.  
...
CPU는 연산 장치다. 데이터가 없다. 메모리(프로세스)는 데이터가 있지만 계산을 수행할 수 없다. 둘 모두 스스로 무언가를 판단할 수 없다. CPU는 메모리로부터 명령을 받아 실행한다. 스스로 무엇을 실행할지 결정할 수 없다. 메모리는 무엇을 실행할지 알고 있지만 실제로 실행할 수 없다. 때문에, 프로세스가 일단 CPU를 점거하면 독재가 가능하다. 프로세스가 가진 명령이 다른 프로세스를 모두 무시하고 영원히 자신만 실행하는 것이라면 운영체제도 개입할 수 없다.(운영체제 역시 프로세스다.) CPU라는 자원을 선점(빼앗는)하지 못한다는 뜻이다. 여러 프로세스가 CPU를 사용하길 기다리고 있다. 어느 한 프로세스가 CPU를 사용중이면 다른 프로세스는 실행중인 프로세스가 끝나길 **기다려야 한다**. 이 같은 방식을 **비선점(non-preemptive)** 라고 한다. 

독재정치는 괴롭다. 많은 시민들이 굶어죽는다(starvation). 앞의 프로세스가 CPU를 오래 붙들고 있으면 뒤의 프로세스는 실행 기회가 오지 않는다. _이런 현상을 기아현상(starvation)이라고 한다._ 프로세스들이 권리를 주장하며 새로운 제도를 만들었다. 민주정부 수립이다. 선거를 통해 정부를 새로 구성하듯이, CPU를 사용하는 프로세스도 외부 요인으로 교체된다. 이처럼 CPU 자원을 선점(빼앗을)할 수 있는 방식을 선점형(preemptive)이라고 부른다. 과정의 핵심은 [인터럽트](#interrupt)다.  
...
사이좋게 프로세스가 CPU를 나눠갖고 있다. 평화가 오는 듯했으나 불만이 생기기 시작했다. 모두 똑같이 시간을 나눠 쓰다보니, 주어진 시간을 다 활용하지 못하는 프로세스가 생겼다. 1분을 줬으면 1분동안 CPU를 열심히 사용해야 하는데, 1초만 쓰고 59초는 CPU랑 노닥거리는 프로세스가 있었다. 커널이 가만 살펴보니 CPU가 놀고 있는 이유는 IO때문이었다. 디스크로부터 정보를 불러오거나 네트워크를 통해 정보를 주고받는 경우 등이었다.  
IO이란 용어는 `Input and Output`으로, 입출력을 의미한다. `IO`, `I/O` 등으로 표기한다. IO은 컴퓨터 시스템에서 필수적이다. CPU가 계산만 하면 뭘 하나. 화면에 보이거나 프린트하거나 디스크에 저장하거나 네트워크 통신을 해야지.  
프로세스가 IO를 하려면 어떻게 해야 할까? IO장치는 하드웨어 장치다. 키보드, 마우스, 모니터, 네트워크, 디스크 모두 IO장치다. 컴퓨터 시스템에서 하드웨어를 관장하는 것은 무엇? 커널이다. 유저 프로세스는 하드웨어에 직접 접근할 수 없다. IO이 필요하면 [시스템 콜](#system-call)을 통해서 커널에 **요청** 해야 한다. 프로세스가 시스템 콜을 호출하면, 스스로 CPU를 내어놓고 커널이 작업을 수행할 수 있도록 한다. _(이처럼 인터럽트가 아닌 자발적으로 커널에게 CPU를 건네는 것을 **Trap** 이라고 한다)_  
커널은 CPU를 건네받아, 해당 하드웨어 드라이버에 IO을 지시한다. 드라이버는 열심히 하드웨어를 돌린다. IO이 끝나면 인터럽트를 발생시킨다. 커널은 인터럽트를 보고 IO이 끝났음을 알게된다. 이어서 IO 데이터를 메모리에 로드(입력의 경우)하고 프로세스에 데이터를 넘겨준다. 간략하게만 봤는데도 오래 걸릴 것 같다. IO장치는 CPU에 비해 백만배 이상 느리다. 더구나 CPU를 커널에게 주었다가 다시 가져와야 하므로 [컨텍스트 스위칭](#context-switching)이 발생한다. 프로세스 입장에서는 아~주 오래 걸린다. 그럼 이렇게 오랫동안 기다리는 프로세스는 그동안 무얼 하고 있었을까? 죽은듯이 기다리고 있었다. CPU를 내어주고 나서 커널이 IO을 수행하는 동안 아무것도 할 수 없었다. 아주 긴 IO 과정을 대기열에서 기다리고 있어야 했다.

IO은 느리다. 이 느린 IO를 프로세스는 꼭(99.99%) 해야 한다. 때문에 거의 대부분의 컴퓨터 or 서버 성능은 IO에 의해 좌우된다. 운영체제는 빠른 IO를 위해 여러 방법들을 고민했다. 디스크를 더 빨리 돌려야 할까? 네트워크 신호를 빛보다 빠르게 전송해야 할까? 운영체제가 **IO 자체**를 개선할 수 있는 건 없었다. 
